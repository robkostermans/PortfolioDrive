...
export const Car = () => {
  ...
  useEventListener("keydown", ({ key }) => {
    console.log(key);
    if (key === "ArrowLeft") start(-1, "turn");
    else if (key === "ArrowRight") start(1, "turn");
    else if (key === "ArrowUp") start(1, "move");
    else if (key === "ArrowDown") start(-1, "move"); // Down
  });

  const t = useTween("inQuad");
  useEffect(() => {
  ...
  const start = (dir, type) => {
    if (type === "turn") startTurn(dir, type);
  ...
  const startTurn = (dir, type) => {
    //setAngle(angle + dir * 10);
    const a = new TWEEN.Tween(turn)
      .to({ speed: 1 * dir }, 500)
      .easing(TWEEN.Easing.Quadratic.In)
      .start();

    if (turn.tween) turn.tween.stop();

    setTurn({
      ...turn,
      angle: turn.angle + dir * 10
    });

    // useAnimationFrame(dir => {
    //   setAngle(angle + dir * 10);
